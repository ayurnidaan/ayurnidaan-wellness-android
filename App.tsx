...
function parseVikritiConclusion(content: string) {
  ...
  return { conclusion: `Your ${option.label} ${option.doshas.length === 1 ? 'dosha is' : 'doshas are'} imbalanced`, doshas: option.doshas, symptoms };
}
...
function CurrentHealthChat({ session, onBack, onComplete }: { session: Session | null; onBack: () => void; onComplete: () => void }) {
  ...
  async function requestReply(conversation: ChatMessage[], forceConclusion = false) {
    ...
    if (forceConclusion && (!result || !result.symptoms.length)) { setSending(false); setError('The assessment did not return a valid conclusion and symptom list. Please try again.'); return; }
    const completedConversation: ChatMessage[] = [...conversation, { role: 'assistant', content: result ? `${result.conclusion}\nSymptoms: ${result.symptoms.join(' | ')}` : reply }];
...
const styles = StyleSheet.create({
  ...
  questionPage: { flex: 1, paddingBottom: 28, paddingHorizontal: 24, paddingTop: 18 }, assessmentPageTitle: { color: '#202921', fontFamily: serif, fontSize: 24, fontWeight: '700' }, questionCount: { color: '#6D756F', fontSize: 13, fontWeight: '600', marginTop: 20 }, progressTrack: { backgroundColor: '#E2E3DB', borderRadius: 4, height: 6, marginTop: 10, overflow: 'hidden' }, progressFill: { backgroundColor: '#075A3F', borderRadius: 4, height: '100%' }, questionPrompt: { color: '#26312B', fontFamily: serif, fontSize: 24, fontWeight: '700', lineHeight: 31, marginBottom: 18, marginTop: 24 }, answerList: { flex: 1, gap: 12 }, answerOption: { alignItems: 'center', backgroundColor: '#FFFEFA', borderColor: '#DEDED4', borderRadius: 14, borderWidth: 1, flex: 1, flexDirection: 'row', justifyContent: 'space-between', maxHeight: 140, minHeight: 96, overflow: 'hidden', paddingRight: 14 }, answerOptionSelected: { backgroundColor: '#F3F8F4', borderColor: '#075A3F', borderWidth: 1.5 }, answerImage: { alignSelf: 'stretch', backgroundColor: '#E8EDE7', height: '100%', marginRight: 13, width: 116 }, answerTextWrap: { alignItems: 'center', flex: 1, flexDirection: 'row', gap: 9, paddingVertical: 12 }, answerLetter: { color: '#075A3F', fontSize: 13, fontWeight: '800' }, answerLabel: { color: '#38423C', flex: 1, fontSize: 13, fontWeight: '600', lineHeight: 18 }, radio: { alignItems: 'center', borderColor: '#C8CBC5', borderRadius: 10, borderWidth: 1.5, height: 20, justifyContent: 'center', marginLeft: 8, width: 20 }, radioSelected: { borderColor: '#075A3F' }, radioDot: { backgroundColor: '#075A3F', borderRadius: 5, height: 10, width: 10 }, questionActions: { flexDirection: 'row', gap: 12, paddingTop: 18 }, continueHalf: { flex: 1 }, answerHint: { color: '#8A6D2F', fontSize: 11, marginTop: 9, textAlign: 'right' },
