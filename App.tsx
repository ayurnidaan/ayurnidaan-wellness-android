...
type Screen = 'splash' | 'intro' | 'auth' | 'account' | 'profile' | 'confirmation' | 'home' | 'prakriti' | 'currentHealth' | 'doctor';
...
export default function App() {
  ...
  if (screen === 'doctor') return <DoctorFlow onExit={() => setScreen('home')} />;
  return <HomeScreen session={session} onStartPrakriti={() => setScreen('prakriti')} onStartCurrentHealth={() => setScreen('currentHealth')} onOpenDoctor={() => setScreen('doctor')} />;
}
...
function HomeScreen({ session, onStartPrakriti, onStartCurrentHealth, onOpenDoctor }: { session: Session | null; onStartPrakriti: () => void; onStartCurrentHealth: () => void; onOpenDoctor: () => void }) {
  const fullName = session?.user.user_metadata.full_name?.trim();
  const firstName = fullName?.split(/\s+/)[0] || 'there';
...
type Doctor = { name: string; initials: string; qualification: string; experience: string; specialty: string; fee: number; rating: string; about: string };
const doctors: Doctor[] = [
  { name: 'Dr. Ananya Sharma', initials: 'AS', qualification: 'BAMS, MD (Ayu)', experience: '8 yrs experience', specialty: 'Ayurveda', fee: 600, rating: '4.8', about: 'Expert in Ayurveda, lifestyle disorders, PCOS, gut health, and personalized wellness care.' },
  { name: 'Dr. Rahul Verma', initials: 'RV', qualification: 'BAMS, MD (Ayu)', experience: '10 yrs experience', specialty: 'Panchakarma', fee: 700, rating: '4.7', about: 'Specialist in Panchakarma therapies, pain management, and restorative Ayurvedic care.' },
  { name: 'Dr. Meera Nair', initials: 'MN', qualification: 'BAMS', experience: '6 yrs experience', specialty: 'Ayurveda', fee: 500, rating: '4.6', about: 'Ayurveda physician focused on nutrition, women’s wellness, and sustainable daily routines.' },
];

function DoctorFlow({ onExit }: { onExit: () => void }) {
