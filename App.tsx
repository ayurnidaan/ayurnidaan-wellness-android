...
function ProfileHub({ session, onExit, onOpenShop, onOpenDoctor, onLogout }: { session: Session | null; onExit: () => void; onOpenShop: () => void; onOpenDoctor: () => void; onLogout: () => Promise<void> }) {
  ...
  const [view, setView] = useState<'profile' | 'orders' | 'order'>('profile');
  ...
  const rows = [{ icon: '⚙', label: 'Preferences' }, { icon: '◉', label: 'Account Settings' }, { icon: '▣', label: 'My Orders', onPress: openOrders }, { icon: '▦', label: 'My Appointments' }, { icon: '?', label: 'Help & Support' }];
