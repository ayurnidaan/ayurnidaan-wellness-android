...
function AuthScreen({ onBack, onAuthenticated }: { onBack: () => void; onAuthenticated: (session: Session) => Promise<void> }) {
  ...
  return <ScreenFrame scroll alignTop>
    ...
    <View style={styles.form}><Text style={styles.onboardingLabel}>MOBILE NUMBER</Text><View style={styles.phoneInputWrap}><Text style={styles.phonePrefix}>+91</Text><View style={styles.phoneDivider} /><TextInput value={mobile} onChangeText={setMobile} placeholder="98765 43210" placeholderTextColor="#26352F" keyboardType="phone-pad" style={styles.phoneInput} /></View>
      <PrimaryButton label="Continue" onPress={() => comingSoon('Mobile number')} onboarding />
    </View>
    <Divider label="or continue with" />
    <View style={styles.socialRow}>
      <SocialButton symbol="G" label="Google" loading={loading} onPress={signInWithGoogle} />
      <SocialButton symbol="A" label="Apple" onPress={() => comingSoon('Apple')} />
...
function ProfileScreen({ session, onBack, onComplete }: { session: Session | null; onBack: () => void; onComplete: () => void }) {
  ...
  async function save() {
    ...
    const { error: saveError } = await supabase.from('profiles').upsert({ user_id: session.user.id, full_name: session.user.user_metadata.full_name ?? null, date_of_birth: normalizedDob, sex, height_cm: Number(height), weight_kg: Number(weight), profile_completed_at: new Date().toISOString() });
...
function ConfirmationScreen({ session, onContinue }: { session: Session | null; onContinue: () => void }) {
  ...
  return <SafeAreaView style={styles.welcomeSafe}><StatusBar style="light" />
    <View style={styles.confirmationContent}>
      ...
      <Text style={styles.confirmationCopy}>Two short assessments and Ayurnidaan{`\n`}can tailor food, yoga and treatment{`\n`}to you.</Text>
    </View>
    <View style={styles.confirmationAction}><Pressable onPress={onContinue} style={styles.homeButton}><Text style={styles.homeButtonText}>Go to home</Text></Pressable></View>
  </SafeAreaView>;
}
      ...
      <View style={styles.answerList}>
        {(Object.entries(question.options) as [AssessmentAnswer, string][]).map(([key, label]) => {
          ...
          return <Pressable key={key} accessibilityRole="radio" accessibilityState={{ selected }} onPress={() => chooseAnswer(key)} style={[styles.answerOption, selected && styles.answerOptionSelected]}>
            <Image source={assessmentOptionImages[questionIndex][key]} style={styles.answerImage} resizeMode="cover" />
...
function parseVikritiConclusion(content: string) {
  ...
  const symptomMatch = content.match(/Symptoms\s*:\s*([^\r\n]+)/i);
  const symptoms = (symptomMatch?.[1] ?? '').split('|').map(symptom => symptom.trim()).filter(Boolean);
  const reasoningMatch = content.match(/Reasoning\s*:\s*([^\r\n]+)/i);
  const reasoning = (reasoningMatch?.[1] ?? '').replace(/\s+/g, ' ').trim();
  return { conclusion: `Your ${option.label} ${option.doshas.length === 1 ? 'dosha is' : 'doshas are'} imbalanced`, doshas: option.doshas, symptoms, reasoning };
}
...
const currentHealthDomains = ['hunger', 'thirst', 'sleep', 'stool', 'urine', 'sweat'] as const;
type CurrentHealthDomain = typeof currentHealthDomains[number];
type CurrentHealthQuestionPhase = 'complaint' | 'domain';
...
function CurrentHealthChat({ session, onBack, onComplete }: { session: Session | null; onBack: () => void; onComplete: () => void }) {
  ...
  const [complaintFollowUpCount, setComplaintFollowUpCount] = useState(0);
  ...
  async function requestReply(conversation: ChatMessage[], forceConclusion = false, questionPhase: CurrentHealthQuestionPhase = 'complaint', requiredDomain?: CurrentHealthDomain) {
    setSending(true); setError('');
    const { data, error: functionError } = await supabase.functions.invoke('current-health-chat', { body: { messages: conversation.slice(1), forceConclusion, questionPhase, requiredDomain } });
    ...
    if (!forceConclusion && result) { setSending(false); setError('The assessment tried to conclude too early. Please try again.'); return; }
    if (forceConclusion && (!result || !result.symptoms.length || !result.reasoning)) { setSending(false); setError('The assessment did not return a valid conclusion, symptom list, and reasoning. Please try again.'); return; }
    const completedConversation: ChatMessage[] = [...conversation, { role: 'assistant', content: result ? `${result.conclusion}\nSymptoms: ${result.symptoms.join(' | ')}\nReasoning: ${result.reasoning}` : reply }];
    setMessages(completedConversation); setSending(false);
    if (!result) {
      if (questionPhase === 'complaint') setComplaintFollowUpCount(count => Math.min(count + 1, 3));
      else setDomainQuestionIndex(index => Math.min(index + 1, currentHealthDomains.length));
      return;
    }
    setConclusionRequired(false);
    if (!session?.user.id) { setError('Please sign in again before saving your assessment.'); return; }
    const { error: saveError } = await supabase.from('current_health_assessments').insert({ user_id: session.user.id, symptoms: result.symptoms, conclusion: result.conclusion, vata_imbalanced: result.doshas.includes('Vata'), pitta_imbalanced: result.doshas.includes('Pitta'), kapha_imbalanced: result.doshas.includes('Kapha'), conversation: completedConversation });
  ...
  function send() {
    ...
    if (complaintFollowUpCount < 3) {
      setMessages(next); requestReply(next, false, 'complaint'); return;
    }
    const requiredDomain = currentHealthDomains[domainQuestionIndex];
    if (requiredDomain) {
      setMessages(next); requestReply(next, false, 'domain', requiredDomain); return;
    }
    if (domainQuestionIndex >= currentHealthDomains.length) {
      setMessages([...next, { role: 'assistant', content: finalComplaintQuestion }]); setAwaitingFinalComplaint(true); return;
    }
  }
  function retry() {
    if (conclusionRequired) { requestReply(messages, true); return; }
    if (complaintFollowUpCount < 3) { requestReply(messages, false, 'complaint'); return; }
    const requiredDomain = currentHealthDomains[domainQuestionIndex];
    if (requiredDomain) requestReply(messages, false, 'domain', requiredDomain);
  }
...
function HomeScreen({ session, onStartPrakriti, onStartCurrentHealth, onOpenDoctor, onOpenShop, onOpenProfile, onOpenAI }: { session: Session | null; onStartPrakriti: () => void; onStartCurrentHealth: () => void; onOpenDoctor: () => void; onOpenShop: () => void; onOpenProfile: () => void; onOpenAI: () => void }) {
  ...
  return <SafeAreaView style={styles.homeSafe}>
    ...
    <View style={styles.bottomNav}>
      <NavItem icon="⌂" label="Home" active /><NavItem icon="✚" label="Doctor" onPress={onOpenDoctor} />
      <NavItem icon="♧" label="AI" onPress={onOpenAI} /><NavItem icon="🛍" label="Shop" onPress={onOpenShop} /><NavItem icon="♙" label="Profile" onPress={onOpenProfile} />
    </View>
  </SafeAreaView>;
}
...
type ProfileHealthResult = { conclusion: string | null; symptoms: string[] | null; vata_imbalanced: boolean; pitta_imbalanced: boolean; kapha_imbalanced: boolean };
...
function ProfileHub({ session, onExit, onOpenShop, onOpenDoctor, onOpenAI, onLogout }: { session: Session | null; onExit: () => void; onOpenShop: () => void; onOpenDoctor: () => void; onOpenAI: () => void; onLogout: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('prakriti_assessments').select('vata_percentage, pitta_percentage, kapha_percentage').eq('user_id', session.user.id).order('completed_at', { ascending: false }).limit(1).maybeSingle(),
      supabase.from('current_health_assessments').select('conclusion, symptoms, vata_imbalanced, pitta_imbalanced, kapha_imbalanced').eq('user_id', session.user.id).order('completed_at', { ascending: false }).limit(1).maybeSingle(),
    ]).then(([prakritiResult, healthResult]) => {
      if (prakritiResult.data) setProfilePrakriti(getProfilePrakriti(prakritiResult.data));
  ...
  const symptomSummary = profileHealth?.symptoms?.length ? profileHealth.symptoms.join(' • ') : 'Complete your current health assessment to see key symptoms.';
  return <SafeAreaView style={styles.profileHubSafe}><StatusBar style="dark" /><ScrollView contentContainerStyle={styles.profileHubScroll}><Text style={styles.profileHubTitle}>My Health Profile</Text><Pressable onPress={openEditProfile} style={styles.profileIdentity}><View style={styles.profileAvatar}>{avatarUrl ? <Image source={{ uri: avatarUrl }} style={styles.profileAvatarImage} /> : <Text style={styles.profileAvatarIcon}>♙</Text>}</View><View style={styles.profileIdentityCopy}><Text style={styles.profileName}>{fullName}</Text><Text style={styles.profileEdit}>View & edit profile</Text></View><Text style={styles.profileChevron}>›</Text></Pressable><View style={styles.healthProfileCards}><View style={styles.healthProfileCard}><View style={styles.healthCardLabelWrap}><Text style={styles.healthCardEyebrow}>YOUR</Text><Text numberOfLines={1} style={styles.healthCardLabel}>Prakriti</Text></View><View style={styles.healthCardContent}><Text style={styles.healthCardTitle}>{profilePrakriti ? `You have a ${profilePrakriti} Prakriti` : 'Complete your Prakriti assessment'}</Text><Text style={styles.healthCardDescription}>{profilePrakriti ? prakritiDescriptions[profilePrakriti] : 'Your constitutional profile and natural tendencies will appear here.'}</Text></View></View><View style={[styles.healthProfileCard, styles.vikritiProfileCard]}><View style={styles.healthCardContent}><Text style={styles.healthCardTitle}>{profileHealth?.conclusion ?? 'Your current balance is not assessed yet'}</Text><Text style={styles.healthSymptomsLabel}>KEY SYMPTOMS</Text><Text style={styles.healthCardDescription}>{symptomSummary}</Text></View><View style={[styles.healthCardLabelWrap, styles.vikritiLabelWrap]}><Text style={[styles.healthCardEyebrow, styles.vikritiLabelText]}>YOUR</Text><Text numberOfLines={1} style={[styles.healthCardLabel, styles.vikritiLabelText]}>Vikriti</Text></View></View><View style={styles.vulnerabilityCard}><Text style={styles.vulnerabilityEyebrow}>DISEASE VULNERABILITY</Text><Text style={styles.vulnerabilityText}>{vikriti ? `You're most likely vulnerable to ${diseaseVulnerability[vikriti]}.` : 'Complete both health assessments to view your vulnerability pattern.'}</Text><Text style={styles.vulnerabilityNote}>This is general wellness guidance, not a medical diagnosis.</Text></View></View><View style={styles.profileDivider} /><Text style={styles.profileOptionsTitle}>Profile & account</Text><View style={styles.profileMenu}>{rows.map(row => <Pressable key={row.label} accessibilityRole="button" onPress={row.onPress} style={({ pressed }) => [styles.profileMenuRow, pressed && styles.pressed]}><View style={styles.profileMenuIcon}><Text style={styles.profileMenuIconText}>{row.icon}</Text></View><Text style={styles.profileMenuLabel}>{row.label}</Text><Text style={styles.profileChevron}>›</Text></Pressable>)}</View><Pressable disabled={loggingOut} onPress={logout} style={({ pressed }) => [styles.logoutRow, pressed && styles.pressed]}><View style={styles.profileMenuIcon}><Text style={styles.profileMenuIconText}>⊖</Text></View>{loggingOut ? <ActivityIndicator color="#7B3939" /> : <Text style={styles.logoutLabel}>Logout</Text>}</Pressable></ScrollView><View style={styles.bottomNav}><NavItem icon="⌂" label="Home" onPress={onExit} /><NavItem icon="✚" label="Doctor" onPress={onOpenDoctor} /><NavItem icon="♧" label="AI" onPress={onOpenAI} /><NavItem icon="🛍" label="Shop" onPress={onOpenShop} /><NavItem icon="♙" label="Profile" active /></View></SafeAreaView>;
}
...
type Doctor = { name: string; initials: string; portrait: number; qualification: string; experience: string; specialty: string; fee: number; rating: string; about: string };
const doctors: Doctor[] = [
  { name: 'Dr Nikhil Khatana', initials: 'NK', portrait: require('./doc-dp/nikhil-khatana.jpeg'), qualification: 'BAMS, MD (Ayu)', experience: '8 yrs experience', specialty: 'Ayurveda', fee: 600, rating: '4.8', about: 'Expert in Ayurveda, lifestyle disorders, gut health, and personalized wellness care.' },
  { name: 'Dr Lalit Madawat', initials: 'LM', portrait: require('./doc-dp/lalit-madawat.jpeg'), qualification: 'BAMS, MD (Ayu)', experience: '10 yrs experience', specialty: 'Panchakarma', fee: 700, rating: '4.7', about: 'Specialist in Panchakarma therapies, pain management, and restorative Ayurvedic care.' },
  { name: 'Dr Nirmal Kumavat', initials: 'NK', portrait: require('./doc-dp/nirmal-kumavat.jpeg'), qualification: 'BAMS', experience: '6 yrs experience', specialty: 'Ayurveda', fee: 500, rating: '4.6', about: 'Ayurveda physician focused on nutrition and sustainable daily routines.' },
];
