...
function ProfileHub({ session, onExit, onOpenShop, onOpenDoctor, onOpenAI, onLogout }: { session: Session | null; onExit: () => void; onOpenShop: () => void; onOpenDoctor: () => void; onOpenAI: () => void; onLogout: () => Promise<void> }) {
  ...
  const [view, setView] = useState<'profile' | 'edit' | 'orders' | 'order' | 'appointments' | 'appointment'>('profile');
  ...
  async function openEditProfile() {
    ...
    const { data, error } = await supabase.from('profiles').select('full_name, date_of_birth, sex, height_cm, weight_kg').eq('user_id', session.user.id).maybeSingle();
