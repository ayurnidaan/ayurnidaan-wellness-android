...
type Screen = 'splash' | 'intro' | 'auth' | 'account' | 'profile' | 'confirmation' | 'home' | 'prakriti' | 'currentHealth' | 'doctor' | 'shop' | 'profileHub';
...
export default function App() {
  ...
  if (screen === 'doctor') return <DoctorFlow session={session} onExit={() => setScreen('home')} onOpenShop={() => setScreen('shop')} onOpenProfile={() => setScreen('profileHub')} />;
  if (screen === 'shop') return <ShopFlow session={session} onExit={() => setScreen('home')} onOpenDoctor={() => setScreen('doctor')} onOpenProfile={() => setScreen('profileHub')} />;
  if (screen === 'profileHub') return <ProfileHub session={session} onExit={() => setScreen('home')} onOpenShop={() => setScreen('shop')} onOpenDoctor={() => setScreen('doctor')} onLogout={async () => { await supabase.auth.signOut(); setSession(null); setScreen('intro'); }} />;
  return <HomeScreen session={session} onStartPrakriti={() => setScreen('prakriti')} onStartCurrentHealth={() => setScreen('currentHealth')} onOpenDoctor={() => setScreen('doctor')} onOpenShop={() => setScreen('shop')} onOpenProfile={() => setScreen('profileHub')} />;
}
...
function HomeScreen({ session, onStartPrakriti, onStartCurrentHealth, onOpenDoctor, onOpenShop, onOpenProfile }: { session: Session | null; onStartPrakriti: () => void; onStartCurrentHealth: () => void; onOpenDoctor: () => void; onOpenShop: () => void; onOpenProfile: () => void }) {
  const fullName = session?.user.user_metadata.full_name?.trim();
  const firstName = fullName?.split(/\s+/)[0] || 'there';
  ...
  return <SafeAreaView style={styles.homeSafe}>
    ...
    <View style={styles.bottomNav}>
      ...
      <NavItem icon="✚" label="Doctor" onPress={onOpenDoctor} /><NavItem icon="♧" label="AI" /><NavItem icon="♙" label="Profile" onPress={onOpenProfile} />
    </View>
  </SafeAreaView>;
}
...
function ProfileHub({ session, onExit, onOpenShop, onOpenDoctor, onLogout }: { session: Session | null; onExit: () => void; onOpenShop: () => void; onOpenDoctor: () => void; onLogout: () => Promise<void> }) {
  const [loggingOut, setLoggingOut] = useState(false);
  const [view, setView] = useState<'profile' | 'orders' | 'order' | 'appointments' | 'appointment'>('profile');
...
function ShopFlow({ session, onExit, onOpenDoctor, onOpenProfile }: { session: Session | null; onExit: () => void; onOpenDoctor: () => void; onOpenProfile: () => void }) {
  ...
  async function checkout() {
    ...
    const { data: order, error: orderInsertError } = await supabase.from('shop_orders').insert({ user_id: session.user.id, total_amount: cartTotal, delivery_postcode: '333031' }).select('id').single();
...
function DoctorFlow({ session, onExit, onOpenShop, onOpenProfile }: { session: Session | null; onExit: () => void; onOpenShop: () => void; onOpenProfile: () => void }) {
