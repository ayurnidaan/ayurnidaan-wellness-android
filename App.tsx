...
function IncompleteHome({ firstName, hasPrakriti, appointment, onStartPrakriti, onStartCurrentHealth, onOpenFood, onOpenYoga, onOpenDoctor, onOpenShop, onOpenProfile, onOpenAI }: { firstName: string; hasPrakriti: boolean; appointment: { doctor_name: string; doctor_initials: string; appointment_date: string; appointment_time: string } | null; onStartPrakriti: () => void; onStartCurrentHealth: () => void; onOpenFood: () => void; onOpenYoga: () => void; onOpenDoctor: () => void; onOpenShop: () => void; onOpenProfile: () => void; onOpenAI: () => void }) {
  ...
  return <SafeAreaView style={styles.homePreSafe}>
    ...
    <ScrollView contentContainerStyle={styles.homePreScroll} showsVerticalScrollIndicator={false}>
      <View style={styles.homePreHeader}><View><Text style={styles.homePreEyebrow}>{greeting}</Text><Text style={styles.homePreName}>{firstName}</Text></View><Pressable accessibilityLabel="Open profile" onPress={onOpenProfile} style={styles.homePreAvatar}><Text style={styles.homePreAvatarText}>{initial}</Text></Pressable></View>
    ...
    <View style={styles.homePreBottomNav}><HomePreNav label="Home" active /><HomePreNav label="Doctor" onPress={onOpenDoctor} /><HomePreNav label="AI" onPress={onOpenAI} /><HomePreNav label="Food" onPress={onOpenFood} /><HomePreNav label="Shop" onPress={onOpenShop} /></View>
...
function ProfileBodyGoalsCard({ metrics, healthGoals }: { metrics: { dateOfBirth: string | null; height: number | null; sex: string | null; weight: number | null }; healthGoals: string[] }) {
  ...
  const bmiMeta = bmiValue === null
    ? { color: '#8CA098', label: 'BMI unavailable', copy: 'Add your height and weight to see your BMI range.' }
    : bmiValue < 18.5
      ? { color: '#73A7A2', label: 'Below healthy range', copy: 'Your BMI is below the general healthy range for adults.' }
      : bmiValue < 25
        ? { color: '#43825F', label: 'Healthy weight', copy: 'Within the general healthy range for your height.' }
        : bmiValue < 30
          ? { color: '#D09A38', label: 'Above healthy range', copy: 'Your BMI is above the general healthy range for adults.' }
          : { color: '#BC725C', label: 'High BMI', copy: 'Your BMI is in the high range; consider discussing it with a clinician.' };
...
const termsConsentStyles = StyleSheet.create({
  ...
  termsZoomLabel: { color: '#819089', fontSize: 6, fontWeight: '700', letterSpacing: 1 },
  ...
  termsZoomResetText: { color: '#527066', fontSize: 7, fontWeight: '700' },
...
const foodStyles = StyleSheet.create({
  ...
  foodMealChange: { color: '#398064', fontSize: 8, marginTop: 11 },
  foodSuggestionPanel: { borderTopColor: '#E0D9CD', borderTopWidth: 1, marginHorizontal: 14, paddingBottom: 14, paddingTop: 12 },
  foodSuggestionEyebrow: { color: '#89978F', fontSize: 7, fontWeight: '700', letterSpacing: 1.5, marginBottom: 8 },
  ...
  foodSuggestionTitle: { color: '#31483D', fontSize: 10, fontWeight: '600' },
  foodSuggestionDescription: { color: '#7B8982', fontSize: 8, lineHeight: 12, marginTop: 4 },
  ...
  foodAiButtonText: { color: '#FFF', fontSize: 9, fontWeight: '700' },
...
const homeV2Styles = StyleSheet.create({
  ...
  scanPermissionText: { color: '#9EB3A9', fontSize: 10, lineHeight: 16, textAlign: 'center' },
  ...
  scanPermissionButtonText: { color: '#F6E2B4', fontSize: 9, fontWeight: '700' },
